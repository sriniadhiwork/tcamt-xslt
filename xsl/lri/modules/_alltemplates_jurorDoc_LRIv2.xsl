   ...
   <xsl:template match="*">
    ...
    <xsl:template name="main">
        ...
        <xsl:if test="$output = 'ng-tab-html'">
            ...
            <xsl:copy-of
                select="util:tabs('Patient Information', 'Display Verification', $full/fieldset[@id = 'PatientInfo-DV'], 'Incorporate Verification', $full/fieldset[@id = 'PatientInfo-IV'])"/>
            <xsl:copy-of
                select="util:tabs('Lab Results', 'Display Verification', $full/fieldset[@id = 'LabResults-DV'], 'Incorporate Verification', $full/fieldset[@id = 'ResultInfo-IV'])"/>
            <xsl:copy-of
                select="util:tabs('Performing Organization Information', 'Display Verification', $full/fieldset[@id = 'PerformingOrg-DV'], 'Incorporate Verification', $full/fieldset[@id = 'PerformingOrderInfo-IV'])"/>
            <xsl:copy-of
                select="util:tabs('Performing Organization Medical Director Information', 'Display Verification', $full/fieldset[@id = 'PerformingOrgMedDr-DV'], '', '')"/>
            <xsl:copy-of
                select="util:tabs('Specimen Information', 'Display Verification', $full/fieldset[@id = 'SpecimenInfo-DV'], 'Incorporate Verification', $full/fieldset[@id = 'SpecimenInfo-IV'])"/>
            <xsl:copy-of
                select="util:tabs('Order Information', 'Display Verification', $full/fieldset[@id = 'OrderInfo-DV'], 'Incorporate Verification', $full/fieldset[@id = 'OrderInfo-IV'])"/>
        ...
        <fieldset id="LabResults-DV">
            ...
            <xsl:for-each select="$groupedLabResults/lab-results/observations">
                ...
                <xsl:variable name="testName">
                    <xsl:choose>
                        <xsl:when
                            test="(($messageID = 'LRI_5.0_2.1-GU_FRU' or $messageID = 'LRI_5.0_2.1-GU_FRN' or $messageID = 'LRI_5.0_2.1-NG_FRU' or $messageID = 'LRI_5.0_2.1-NG_FRN') and $pos = 2)">
                            <xsl:copy-of
                                select="$er7XMLMessage/ORU_R01/ORU_R01.PATIENT_RESULT/ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION[2]/*"
                            />
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:copy-of
                                select="$er7XMLMessage/ORU_R01/ORU_R01.PATIENT_RESULT/ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION[1]/*"
                            />
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                ...
                <table id="labResultsDisplay">
                    <thead>
                        ...
                        <tr>
                            ...
                            <td colspan="9" width="75%" style="font-weight:normal;">
                                <xsl:value-of
                                    select="util:formatDateTime($testName/OBR/OBR.22/OBR.22.1)"/>
                            </td>
                    ...
                    <tbody>
                        <xsl:for-each
                            select="ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.OBSERVATION/OBX | ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION/ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.OBSERVATION/OBX">
                                    ...
                                    <xsl:when test="OBX.2 = 'SN'">
                                        ...
                                        <xsl:choose>
                                            <xsl:when test="exists(OBX.5)">
                                                <td>
                                                  <xsl:copy-of
                                                  select="concat(concat('&lt;font color=&quot;red&quot;>&lt;b>', OBX.5/OBX.5.1, '&lt;/b>&lt;/font>'), ' ', concat('&lt;font color=&quot;black&quot;>&lt;b>&lt;i>', OBX.5/OBX.5.2, '&lt;/i>&lt;/b>&lt;/font>'), ' ', concat('&lt;font color=&quot;red&quot;>&lt;b>', OBX.5/OBX.5.3, '&lt;/b>&lt;/font>'), ' ', concat('&lt;font color=&quot;black&quot;>&lt;b>&lt;i>', OBX.5/OBX.5.4, '&lt;/i>&lt;/b>&lt;/font>'))"
                                                  />
                                                </td>
                                            </xsl:when>
                                            <xsl:otherwise>
                            ...
                            <tr>
                                ...
                                <xsl:copy-of
                                    select="util:displayCond(OBX.3/OBX.3.2, OBX.3/OBX.3.5, OBX.3/OBX.3.2)"/>
                                <xsl:copy-of select="$resultValue"/>
                                <xsl:copy-of
                                    select="util:displayCond(OBX.6/OBX.6.9, OBX.6/OBX.6.5, OBX.6/OBX.6.2)"/>
                        ...
                        <xsl:if
                            test="$groupedLabResults/lab-results/observations/ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.OBSERVATION/OBX/OBX.2 = 'NM'">
    ...
    <xsl:template name="processLabResults">
        ...
        <xsl:choose>
            <xsl:when
                test="count($er7XMLMessage/ORU_R01/ORU_R01.PATIENT_RESULT/ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION) > 1">
            ...
            <xsl:otherwise>
                <xsl:call-template name="groupFinalLabResults">
                    <xsl:with-param name="groupedResults"
                        select="$er7XMLMessage/ORU_R01/ORU_R01.PATIENT_RESULT/ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION"
                    />
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="groupER7XMLLabResults">
        ...
        <xsl:call-template name="processParentOBR">
            <xsl:with-param name="parentOBR"
                select="$labResults/OBR[count(OBR.26/OBR.26.2/OBR.26.2.3) = 0]/.."/>
    ...
    <xsl:template name="processParentOBR">
        ...
        <xsl:variable name="results">
            <xsl:for-each
                select="$parentOBR/ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.OBSERVATION/OBX/OBX.2[. = 'CWE']">

                <xsl:variable name="currentGroupId" select="../OBX.4/OBX.4.2"/>

                <xsl:copy-of
                    select="$groupedLabRes/OBR/OBR.26/OBR.26.2/OBR.26.2.2[. = $currentGroupId]/../../../../ORC"/>
                <xsl:choose>
                    <xsl:when
                        test="$groupedLabRes/OBR/OBR.26/OBR.26.2/OBR.26.2.2[. = $currentGroupId]/../../../../OBR">
                        <xsl:copy-of
                            select="$groupedLabRes/OBR/OBR.26/OBR.26.2/OBR.26.2.2[. = $currentGroupId]/../../../../OBR"
                        />
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <xsl:copy-of
                    select="$parentOBR/ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.OBSERVATION/OBX/OBX.4/OBX.4.2[. = $currentGroupId]/../../.."/>

                <xsl:copy-of
                    select="$groupedLabRes/OBR/OBR.26/OBR.26.2/OBR.26.2.2[. = $currentGroupId]/../../../../ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.OBSERVATION"
                />
            </xsl:for-each>
        </xsl:variable>
    ...
    <xsl:template name="groupFinalLabResults">
        ...
        <xsl:variable name="finalGroupedResults" as="node()*">
            <lab-results>
                <xsl:for-each-group select="$groupedResults/*"
                    group-starting-with="ORC | ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION">
            ...
            <table id="performingOrganizationNameAdd">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <xsl:copy-of
                            select="util:formatData($obxSegment/OBX.24/OBX.24.1/OBX.24.1.1, 'boldItalic')"/>
                        <xsl:call-template name="commentTemplate"/>
                    </tr>
                    <tr>
                        <th class="embSpace">Other designation</th>
                        <xsl:copy-of
                            select="util:formatData($obxSegment/OBX.24/OBX.24.2, 'boldItalic')"/>
                        <xsl:call-template name="commentTemplate"/>
                    </tr>
                    <tr>
                        <th class="embSpace">City</th>
                        <xsl:copy-of
                            select="util:formatData($obxSegment/OBX.24/OBX.24.3, 'boldItalic')"/>
                        <xsl:call-template name="commentTemplate"/>
                    </tr>
                    <tr>
                        <th class="embSpace">State</th>
                        <xsl:copy-of
                            select="util:formatData($obxSegment/OBX.24/OBX.24.4, 'boldItalic')"/>
                        <xsl:call-template name="commentTemplate"/>
                    </tr>
                    <tr>
                        <th class="embSpace">Zip code</th>
                        <xsl:copy-of
                            select="util:formatData($obxSegment/OBX.24/OBX.24.5, 'boldItalic')"/>
                        <xsl:call-template name="commentTemplate"/>
                    </tr>
                ...
                <table id="orderInformation">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <xsl:choose>
                                <xsl:when
                                    test="exists($orcSegment/following-sibling::OBR[1]/OBR.13/OBR.13.9)">
                                    <xsl:copy-of
                                        select="util:formatData($orcSegment/following-sibling::OBR[1]/OBR.13/OBR.13.9, 'bold')"
                                    />
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:copy-of
                                        select="util:formatData($orcSegment/following-sibling::OBR[1]/OBR.13/OBR.13.2, 'boldItalic')"
                                    />
                                </xsl:otherwise>
                            </xsl:choose>
                            <xsl:call-template name="commentTemplate"/>
                        ...
                        <xsl:if
                            test="exists($orcSegment/following-sibling::ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.TIMING_QTY[1]/TQ1)">
                            <tr>
                            ...
                            <tr>
                                ...
                                <xsl:copy-of
                                    select="util:formatData(util:formatDateTime($orcSegment/following-sibling::ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.TIMING_QTY[1]/TQ1/TQ1.7/TQ1.7.1), 'normal')"/>
                                <xsl:call-template name="commentTemplate"/>
                            </tr>
                            <tr>
                                <th class="embSpace">End Date/Time</th>
                                <xsl:copy-of
                                    select="util:formatData(util:formatDateTime($orcSegment/following-sibling::ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.TIMING_QTY[1]/TQ1/TQ1.8/TQ1.8.1), 'normal')"/>
                                <xsl:call-template name="commentTemplate"/>
                            </tr>
                        </xsl:if>
                    </tbody>
        ...
        <fieldset id="PatientInfo-IV">
            <table id="identifierInformation">
                ...
                <tbody>
                    ...
                    <xsl:copy-of
                        select="util:ID-text-format('PID-3.1', 'ID Number', 'S-EX-A', $pidSegment/PID.3/PID.3.1, '20px', 'normal')"/>
                    <tr>
                    ...
                    <xsl:copy-of
                        select="util:ID-text-format('PID-7.1', 'Time', 'S-EQ', util:parseDate($pidSegment/PID.7/PID.7.1), '20px', 'normal')"/>
                    <xsl:copy-of
                    ...
                    <xsl:for-each select="$pidSegment/PID.10">
                        ...
                        <xsl:copy-of
                            select="util:ID-text-format('PID-10.1', 'Identifier', 'S-RC', PID.10.1, '20px', 'normal')"/>
                        <xsl:copy-of
                            select="util:ID-text-format('PID-10.2', 'Text', 'S-RC', PID.10.2, '20px', 'normal')"/>
                        <xsl:copy-of
                            select="util:ID-text-format('PID-10.3', 'Name of Coding System', 'S-RC', PID.10.3, '20px', 'normal')"
                        />
                    </xsl:for-each>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th> OBR-4 <xsl:if  test="util:isParentChildTestCase($messageID) = true()">
                                    <xsl:value-of select="'(Parent Information)'"/></xsl:if>
                            </th>
                        ...
                        <xsl:for-each select="OBR.28">
                            ...
                            <xsl:copy-of
                                select="util:ID-text-format('OBR-28.2.1', 'Surname', 'S-EX-A', OBR.28.2/OBR.28.2.1, '30px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBR-28.3', 'Given Name', 'S-EX-A', OBR.28.3, '20px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBR-28.4', 'Second and Further Given Names or Initials Thereof', 'S-EX-A', OBR.28.4, '20px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBR-28.5', 'Suffix (e.g., JR or III)', 'S-EX-A', OBR.28.5, '20px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBR-28.6', 'Prefix (e.g., DR)', 'S-EX-A', OBR.28.6, '20px', 'normal')"/>
                        ...
                        <tr>
                            <td colspan="5"><b>Note 1</b> -Store the <u>Identifier</u> and the
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th> OBR-4 <xsl:if
                                    ...
                                    <xsl:value-of select="'(Child Information)'"/></xsl:if>
                            </th>
                            <th>Universal Service Identifier (Note 1)</th>
                            <th/>
                            <th/>
                        ...
                        <xsl:if test="exists(OBR.26)">
                            ...
                            <xsl:copy-of
                                select="util:ID-text-format('OBR-26.2.2', 'Group', 'S-EX-A', OBR.26/OBR.26.2/OBR.26.2.2, '30px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBR-26.2.3', 'Sequence', 'S-EX-A', OBR.26/OBR.26.2/OBR.26.2.3, '30px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBR-26.2.4', 'Identifier', 'S-EX-A', OBR.26/OBR.26.2/OBR.26.2.4, '30px', 'normal')"/>
            ...
            <xsl:for-each select="$obxSegment">
                <table id="identifierInformation">
                    ...
                    <tbody>
                        ...
                        <xsl:copy-of
                            select="util:ID-text-format('OBX-23.1', 'Organization Name (Note 1)', 'S-TR-R', $obxSegment/OBX.23/OBX.23.1, '20px', 'normal')"/>
                        <tr>
    ...
    <xsl:template name="resultInfo-IV">
        ...
        <fieldset id="ResultInfo-IV">
            <xsl:for-each select="$obxSegment/OBX | $obxSegment[name(.) = 'OBR']">
					<xsl:message><xsl:value-of select="position()"/><xsl:value-of select="name(.)"/>bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</xsl:message>
			...
			<xsl:otherwise>
                ...
                <xsl:variable name="resultValue">
                    <xsl:choose>
                        ...
                        <xsl:when test="OBX.2 = 'SN'">
                            ...
                            <xsl:copy-of
                                select="util:ID-text-format('OBX-5.1', 'Comparator', 'S-EX', OBX.5/OBX.5.1, '20px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBX-5.2', 'Num1', 'S-EQ', OBX.5/OBX.5.2, '20px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBX-5.3', 'Separator/Suffix', 'S-EX', OBX.5/OBX.5.3, '20px', 'normal')"/>
                            <xsl:copy-of
                                select="util:ID-text-format('OBX-5.4', 'Num2', 'S-EQ', OBX.5/OBX.5.4, '20px', 'normal')"/>
                ...
                <table id="identifierInformation">
                    ...
                    <tbody>
                        ...
                        <xsl:copy-of
                            select="util:ID-text-format('OBX-6.2', 'Text', 'S-TR-R', OBX.6/OBX.6.2, '20px', 'normal')"/>
                        <xsl:copy-of
                    ...
                    <table id="note">
                        ...
                        <tbody>
                            <xsl:for-each select="following-sibling::NTE">
                                ...
                                <xsl:copy-of
                                    select="util:ID-text-format('NTE-3', 'Note', 'S-EX', util:parseText(NTE.3), '0px', 'normal')"/>
                ...
                <xsl:message><xsl:value-of select="name(../preceding-sibling::*[1])" />877777777777777777777777777777777777777777777777777777777777777777777777777777</xsl:message>

                <xsl:message><xsl:value-of select="name(../following-sibling::*[1])" />977777777777777777777777777777777777777777777777777777777777777777777777777777</xsl:message>

                <xsl:message><xsl:value-of select="count(../following-sibling::ORU_R01.PATIENT_RESULT.ORDER_OBSERVATION.OBSERVATION)" />977777777777777777777777777777777777777777777777777777777777777777777777777777</xsl:message>
